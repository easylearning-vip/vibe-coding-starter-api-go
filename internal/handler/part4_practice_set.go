...
type Part4PracticeSetHandler struct {
	svc    service.Part4PracticeSetService
	logger logger.Logger
}

func NewPart4PracticeSetHandler(svc service.Part4PracticeSetService, logger logger.Logger) *Part4PracticeSetHandler {
	return &Part4PracticeSetHandler{svc: svc, logger: logger}
}

func (h *Part4PracticeSetHandler) RegisterRoutes(r *gin.RouterGroup) {
	s := r.Group("/user/part4-practice-sets")
	{
		s.POST("", h.CreateSet)
	...
	it := r.Group("/user/part4-practice-set-items")
	{
...
func (h *Part4PracticeSetHandler) RegisterPublicRoutes(r *gin.RouterGroup) {
	p := r.Group("/public/toeic/part4")
	{
...
func (h *Part4PracticeSetHandler) PublicListSetQuestions(c *gin.Context) {
	...
	for _, it := range res {
		...
		case 1:
			text = it.Talk.Question1
		case 2:
			text = it.Talk.Question2
		default:
			text = it.Talk.Question3
		}
		var oa, ob, oc, od string
		for _, ao := range it.Talk.AnswerOptions {
...
func (h *Part4PracticeSetHandler) PublicGetItemDetail(c *gin.Context) {
	id, err := strconv.ParseUint(c.Param("id"), 10, 32)
	if err != nil {
		c.JSON(http.StatusBadRequest, ErrorResponse{Error: "invalid_id", Message: "Invalid ID"})
		return
	...
	case 1:
		text = res.Talk.Question1
	case 2:
		text = res.Talk.Question2
	default:
		text = res.Talk.Question3
	}
	var oa, ob, oc, od string
	for _, ao := range res.Talk.AnswerOptions {
...
func (h *Part4PracticeSetHandler) getUserID(c *gin.Context) uint {
...
func (h *Part4PracticeSetHandler) CreateSet(c *gin.Context) {
...
func (h *Part4PracticeSetHandler) AutoGenerate(c *gin.Context) {
	userID := h.getUserID(c)
	if userID == 0 {
		c.JSON(http.StatusUnauthorized, ErrorResponse{Error: "unauthorized", Message: "User not authenticated"})
		return
...
func (h *Part4PracticeSetHandler) GetSetByID(c *gin.Context) {
	userID := h.getUserID(c)
	if userID == 0 {
		c.JSON(http.StatusUnauthorized, ErrorResponse{Error: "unauthorized", Message: "User not authenticated"})
		return
...
func (h *Part4PracticeSetHandler) ListSetQuestions(c *gin.Context) {
	userID := h.getUserID(c)
	if userID == 0 {
		c.JSON(http.StatusUnauthorized, ErrorResponse{Error: "unauthorized", Message: "User not authenticated"})
		return
...
func (h *Part4PracticeSetHandler) UpdateSet(c *gin.Context) {
	userID := h.getUserID(c)
	if userID == 0 {
		c.JSON(http.StatusUnauthorized, ErrorResponse{Error: "unauthorized", Message: "User not authenticated"})
		return
...
func (h *Part4PracticeSetHandler) DeleteSet(c *gin.Context) {
	userID := h.getUserID(c)
	if userID == 0 {
		c.JSON(http.StatusUnauthorized, ErrorResponse{Error: "unauthorized", Message: "User not authenticated"})
		return
...
func (h *Part4PracticeSetHandler) ListSets(c *gin.Context) {
	userID := h.getUserID(c)
	if userID == 0 {
		c.JSON(http.StatusUnauthorized, ErrorResponse{Error: "unauthorized", Message: "User not authenticated"})
		return
...
func (h *Part4PracticeSetHandler) CreateItem(c *gin.Context) {
	userID := h.getUserID(c)
	if userID == 0 {
		c.JSON(http.StatusUnauthorized, ErrorResponse{Error: "unauthorized", Message: "User not authenticated"})
		return
	}
	var req service.CreatePart4SetItemRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, ErrorResponse{Error: "validation_error", Message: err.Error()})
		return
	}
...
func (h *Part4PracticeSetHandler) ListItems(c *gin.Context) {
	userID := h.getUserID(c)
	if userID == 0 {
		c.JSON(http.StatusUnauthorized, ErrorResponse{Error: "unauthorized", Message: "User not authenticated"})
		return
...
func (h *Part4PracticeSetHandler) GetItemDetail(c *gin.Context) {
	userID := h.getUserID(c)
	if userID == 0 {
		c.JSON(http.StatusUnauthorized, ErrorResponse{Error: "unauthorized", Message: "User not authenticated"})
		return
...
func (h *Part4PracticeSetHandler) SubmitAnswer(c *gin.Context) {
	userID := h.getUserID(c)
	if userID == 0 {
		c.JSON(http.StatusUnauthorized, ErrorResponse{Error: "unauthorized", Message: "User not authenticated"})
		return
	...
	var req service.SubmitPart4AnswerRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, ErrorResponse{Error: "validation_error", Message: err.Error()})
		return
	}
...
func (h *Part4PracticeSetHandler) DeleteItem(c *gin.Context) {
	userID := h.getUserID(c)
	if userID == 0 {
		c.JSON(http.StatusUnauthorized, ErrorResponse{Error: "unauthorized", Message: "User not authenticated"})
		return
