...
func (r *part2PracticeSetItemRepository) Create(ctx context.Context, entity *model.Part2PracticeSetItem) error {
    if err := r.db.WithContext(ctx).Create(entity).Error; err != nil { return fmt.Errorf("create Part2PracticeSetItem: %w", err) }
    return nil
}
func (r *part2PracticeSetItemRepository) GetByID(ctx context.Context, id uint) (*model.Part2PracticeSetItem, error) {
    ...
    if err := r.db.WithContext(ctx).First(&e, id).Error; err != nil { return nil, err }
    return &e, nil
}
func (r *part2PracticeSetItemRepository) Update(ctx context.Context, entity *model.Part2PracticeSetItem) error {
    if err := r.db.WithContext(ctx).Save(entity).Error; err != nil { return err }
    return nil
}
...
func (r *part2PracticeSetItemRepository) List(ctx context.Context, opts ListOptions) ([]*model.Part2PracticeSetItem, int64, error) {
    ...
    for k,v := range opts.Filters {
        switch k {
        case "set_id": if vv,ok:=v.(uint); ok && vv>0 { q = q.Where("set_id = ?", vv) }
        case "question_id": if vv,ok:=v.(uint); ok && vv>0 { q = q.Where("question_id = ?", vv) }
        }
    }
    if err := q.Count(&total).Error; err != nil { return nil,0, err }
    if opts.Sort != "" {
        ord := "ASC"; if opts.Order == "desc" { ord = "DESC" }
        q = q.Order(fmt.Sprintf("%s %s", opts.Sort, ord))
    } else { q = q.Order("order_index ASC, id ASC") }
